...
Inputs & Dependencies:
	...
	- [IDI_Community].[cm_read_INCOME_T1_INC_SUPPORT_PAYMT].[income_t1_inc_support_paymt_202410]
...
Description:
Indication of whether a person has had a NEET period in the previous 12 months (?) 
	NEET defined as:
...
Intended purpose:
Understand the number of young people (aged 15-24 years) who are NEET. Young people are usually more severely affected by economic crises than other age groups. 
...
- Main difference to OT's code is the imputation of withdrawal dates from tertiary enrolment
 ...
 Runtime (before joinng to master) - 00:36:50
...
SELECT a.snz_uid
    , a.dob
    , MAX(enddate) AS end_date /* latest date we want to generate for each person */
INTO #input_pop
FROM(
    SELECT *
    FROM [IDI_Sandpit].[DL-MAA2023-55].[master_table_202410]  /*This is a population of 15-24 year olds over time */
    WHERE AGE_RDP = 3
    AND POPULATION = 1
)a
GROUP BY a.snz_uid
...
DROP TABLE IF EXISTS IDI_Sandpit.[DL-MAA2023-55].EET_draft;
SELECT a.snz_uid
    ...
    , 'overseas' AS status
INTO IDI_Sandpit.[DL-MAA2023-55].EET_draft
...
JOIN #input_pop b
ON a.snz_uid = b.snz_uid --where a.cor_rommp_directive_type not in ('AGED_OUT','ALIVE','ERROR','NA') -- might want to limit to exclude community service in future*
WHERE a.cor_rommp_directive_type IN ('IMPRISONMENT', 'REMAND') --, 'HOME DETENTION' 
AND a.cor_rommp_period_start_date < CASE WHEN a.cor_rommp_period_end_date IS NULL THEN '9999-12-31' ELSE a.cor_rommp_period_end_date END
...
--- EET_linked RUN AT LOW DEMAND TIME --

DROP TABLE IF EXISTS IDI_Sandpit.[DL-MAA2023-55].EET_linked;
WITH start_dates AS(
    SELECT snz_uid
        , [start_date]
    FROM IDI_Sandpit.[DL-MAA2023-55].EET_draft a
    WHERE NOT EXISTS(
        SELECT 1
        FROM IDI_Sandpit.[DL-MAA2023-55].EET_draft b
...
INTO IDI_Sandpit.[DL-MAA2023-55].EET_linked
FROM start_dates a
...
INTO #NEET_periods
FROM IDI_Sandpit.[DL-MAA2023-55].EET_linked a
INNER JOIN IDI_Sandpit.[DL-MAA2023-55].EET_linked b
ON a.snz_uid = b.snz_uid
...
SELECT n.snz_uid
    ...
    , CASE WHEN SUM(n.NEET_length_yr) >= 30 THEN 1 ELSE NULL END AS short_term_NEET -- 1 month
...
DROP COLUMN IF EXISTS NEET_6mo_in_yr
    , COLUMN IF EXISTS NEET_1mo_in_qt;
ALTER TABLE [IDI_Sandpit].[DL-MAA2023-55].[master_table_202410] ADD NEET_6mo_in_yr bit
    , NEET_1mo_in_qt bit;
--GO

UPDATE [IDI_Sandpit].[DL-MAA2023-55].[master_table_202410]
SET NEET_6mo_in_yr = CASE WHEN neet.long_term_NEET = 1 AND AGE_RDP = 3 THEN 1 ELSE NULL END
    , NEET_1mo_in_qt = CASE WHEN neet.NEET_qt = 1 AND AGE_RDP = 3 THEN 1 ELSE NULL END
...
UPDATE [IDI_Sandpit].[DL-MAA2023-55].[master_table_202410]
SET NEET_6mo_in_yr_25yr = CASE WHEN neet.long_term_NEET = 1 AND AGE_25YO = 1 THEN 1 ELSE NULL END
    ,  --only take the working age population for the employed indicator
 NEET_1mo_in_qt_25yr = CASE WHEN neet.NEET_qt = 1 AND AGE_25YO = 1 THEN 1 ELSE NULL END
